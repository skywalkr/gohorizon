...
// checks if the DesktopPoolSessionSettingsUpdateSpec type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DesktopPoolSessionSettingsUpdateSpec{}
...
type DesktopPoolSessionSettingsUpdateSpec struct {
	...
	// Power policy for the machines in the desktop pool after logoff. This setting is only relevant for managed machines.  * TAKE_NO_POWER_ACTION: No action will be taken when user logs off. * ALWAYS_POWERED_ON: Ensure machines in the Desktop pool are always powered on. The connection server will monitor and power on machines as necessary. * SUSPEND: Suspend when a user logs off or when desktop pool is no longer keeping a machine as a spare. This does not affect spare and newly provisioned machines. * POWER_OFF: Power off when a user logs off or when desktop pool is no longer keeping a machine as a spare. This does not affect spare and newly provisioned machines.
...
type _DesktopPoolSessionSettingsUpdateSpec DesktopPoolSessionSettingsUpdateSpec
...
func (o DesktopPoolSessionSettingsUpdateSpec) ToMap() (map[string]interface{}, error) {
...
func (o *DesktopPoolSessionSettingsUpdateSpec) UnmarshalJSON(data []byte) (err error) {
	...
	varDesktopPoolSessionSettingsUpdateSpec := _DesktopPoolSessionSettingsUpdateSpec{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varDesktopPoolSessionSettingsUpdateSpec)

	if err != nil {
		return err
	}

	*o = DesktopPoolSessionSettingsUpdateSpec(varDesktopPoolSessionSettingsUpdateSpec)
