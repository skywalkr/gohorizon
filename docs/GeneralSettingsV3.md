...
------------ | ------------- | ------------- | -------------
**ApiSessionTimeoutMinutes** | Pointer to **int32** | Determines how long (in minutes) a refresh token is valid for and an idle View API session continues before the session times out. Setting the API session timeout to a high number of minutes increases the risk of unauthorized use of View API. Use caution when you allow an idle session to persist for a long time. | [optional] 
